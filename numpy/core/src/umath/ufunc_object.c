...
/*
 * Creates an iterator for use in the PyUfunc_Accumulate function. Both
 * arr and out must have matching shapes and matching NBO dtypes and be
 * aligned.
 */

static NpyIter*
get_accumulate_iter(PyArrayObject *out, PyArrayObject *arr, int axis)
{
    PyArrayObject *op[2] = {out, arr};
    PyArray_Descr *op_dtypes[2] = {PyArray_DESCR(out), PyArray_DESCR(out)};
    npy_uint32 flags = NPY_ITER_ZEROSIZE_OK | NPY_ITER_REFS_OK |
                       NPY_ITER_MULTI_INDEX;
    npy_uint32 op_flags[2] = {NPY_ITER_READWRITE | NPY_ITER_NO_BROADCAST |
                              NPY_ITER_ALIGNED | NPY_ITER_NBO,
                              NPY_ITER_READONLY | NPY_ITER_NO_BROADCAST |
                              NPY_ITER_ALIGNED | NPY_ITER_NBO};

    static int op_axes_arrays[2][NPY_MAXDIMS];
    static int init = 0;
    ...
    if (!init) {
        ...
        for (idim = 0; idim < NPY_MAXDIMS; ++idim) {
    ...
    /*
     * The NPY_NO_CASTING flag, combined with both entries of op_dtypes
     * pointing to the dtype of the same array, ensures that both arr
     * and out are of equivalent type.
     */
    iter = NpyIter_AdvancedNew(2, op, flags,
                               NPY_KEEPORDER, NPY_NO_CASTING,
                               op_flags, op_dtypes,
                               PyArray_NDIM(arr), op_axes, NULL, 0);
    if (iter == NULL) {
        return NULL;
    }
                   ...
                   int naxes, int *axes, int otype)
{
    ...
    if (get_binary_op_function(ufunc, &otype_final,
                               &innerloop, &innerloopdata) < 0) {
        dtype = PyArray_DescrFromType(otype);
        PyErr_Format(PyExc_ValueError,
    ...
    /*
     * Set up the output data type, using the input's exact data type
     * if the type number didn't change to preserve metadata.
     */
    if (PyArray_DESCR(arr)->type_num == otype_final) {
        if (PyArray_ISNBO(PyArray_DESCR(arr)->byteorder)) {
            ...
            dtype = PyArray_DescrNewByteorder(PyArray_DESCR(arr), NPY_NATIVE);
        }
    }
    else {
    ...
    /*
     * The inner loop needs both arr and out to be aligned, and of the
     * same NBO dtype. We would normally let the iterator do this for us,
     * but for multiple axes it could lead to many unnecessary data
     * update-if-copys, so we handle it explicitly.
     */
    if (!PyArray_ISALIGNED(arr) ||
            !PyArray_EquivTypes(dtype, PyArray_DESCR(arr))) {
        Py_INCREF(dtype);
        op[1] = (PyArrayObject *)PyArray_FromArray(arr, dtype,
                                                   NPY_ARRAY_ALIGNED |
                                                   NPY_ARRAY_FORCECAST);
        if (op[1] == NULL) {
            goto fail;
    ...
    if (out == NULL) {
        Py_INCREF(dtype);
        out = op[0] = (PyArrayObject *)PyArray_NewFromDescr(
                                    &PyArray_Type, dtype,
                                    ndim, PyArray_DIMS(op[1]),
                                    NULL, NULL, 0, NULL);
        if (op[0] == NULL) {
            goto fail;
        }
        ...
        if (PyArray_NDIM(arr) != PyArray_NDIM(out) ||
            !PyArray_CompareLists(PyArray_DIMS(arr),
                                  PyArray_DIMS(out), ndim)) {
            PyErr_SetString(PyExc_ValueError,
        ...
        if (!PyArray_ISALIGNED(out) ||
                !PyArray_EquivTypes(dtype, PyArray_DESCR(out))) {
            Py_INCREF(dtype);
            op[0] = (PyArrayObject *)PyArray_FromArray(out, dtype,
                                                       NPY_ARRAY_ALIGNED |
    ...
    /*
     * If the array has size zero, return the zero-sized output
     * array for UFUNC_ACCUMULATE.
     */
    if (PyArray_SIZE(op[0]) == 0) {
        goto finish;
    }

    for (idim = 0; idim < naxes; ++idim) {
        int axis = axes[idim];
        char *dataptr_copy[3];
        npy_intp stride_copy[3];
        npy_intp count_m1;
        NpyIter_IterNextFunc *iternext;
        char **dataptr = dataptr_copy;
        int itemsize = dtype->elsize;
        NpyIter *iter = NULL;

        if (ndim > 1) {
            /* Need an outer iterator */
            NPY_UF_DBG_PRINT("UFunc: Reduce loop with just outer iterator\n");
            iter = get_accumulate_iter(op[0], op[1], axis);
            if (iter == NULL) {
                goto fail;
            }
            /* Set up iteration */
        ...
        stride_copy[0] = PyArray_STRIDE(op[0], axis);
        stride_copy[1] = PyArray_STRIDE(op[1], axis);
        stride_copy[2] = stride_copy[0];

        if ((iter && !NpyIter_IterationNeedsAPI(iter)) ||
                (!iter && !PyDataType_REFCHK(dtype))) {
        ...
        do {
            ...
            if (count_m1 > 0) {
                /* Turn the two items into three for the inner loop */
                dataptr_copy[1] += stride_copy[1];
                dataptr_copy[2] += stride_copy[0];
                NPY_UF_DBG_PRINT1("iterator loop count %d\n", (int)count_m1);
                innerloop(dataptr_copy, &count_m1, stride_copy, innerloopdata);
            }
        } while (iter && iternext(iter));
        ...
        if (idim == 0  && naxes > 1) {
            /* For subsequent axes, op[0] is both input and output */
            Py_DECREF(op[1]);
            op[1] = op[0];
            Py_INCREF(op[1]);
        }
                         ...
                         PyObject *kwds, int operation)
{
    ...
    case UFUNC_ACCUMULATE:
        ...
        //     return NULL;
        // }
        ret = (PyArrayObject *)PyUFunc_Accumulate(ufunc, mp, out, naxes, axes,
                                                  otype->type_num);
        break;
    case UFUNC_REDUCEAT:
        if (naxes != 1) {
